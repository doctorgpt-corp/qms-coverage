...
async function main() {
    ...
    if (!response.ok) {
        ...
        throw new Error(`qms-coverage publish: POST ${url}/api/evidence ${response.status}: ${body}`);
    }
    const ok = (await response.json().catch(() => null));
    const recorded = ok?.recorded ?? manifest.results.length;
    const skipped = ok?.skipped ?? [];
    // eslint-disable-next-line no-console
    console.log(`[qms-coverage] pushed ${recorded} evidence row(s) across ${manifest.results.length} test(s) on ${manifest.branch}@${manifest.sha.slice(0, 7)}.`);
    if (skipped.length > 0) {
        process.stderr.write(`[qms-coverage] WARN: ${skipped.length} unknown slug(s) skipped (author them in QMS to start tracking):\n` +
            skipped.map((s) => `  - ${s}\n`).join(""));
    }
}
