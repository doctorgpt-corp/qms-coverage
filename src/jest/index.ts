...
function getCurrentFullName(fallback: string): string {
  try {
...
function recordSlugs(fullName: string, slugs: readonly string[]): void {
  ...
  appendFileSync(
    path,
    `${JSON.stringify({ fullName, slugs: [...slugs] })}\n`,
    "utf8",
  );
}

function validate(name: string, slugs: readonly string[]): void {
  ...
  for (const s of slugs) {
    if (typeof s !== "string" || !SLUG_RE.test(s)) {
      throw new Error(
        `qms-coverage: test("${name}") has invalid slug ${JSON.stringify(s)}. ` +
